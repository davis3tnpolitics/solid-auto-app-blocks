...
function scaffoldNextApp({ appName, targetDir, packages, port, includeSample, force }) {
  const files = {
    ...
    "eslint.config.mjs": createEslintConfig(),
...
- ESLint uses flat config with Next core-web-vitals + typescript presets.
- A simple \`/api/health\` route is included for quick smoke tests.
- App CI workflow is generated at \`.github/workflows/app-${appName}-ci.yml\` by default (\`--skip-ci-workflow\` to opt out).
